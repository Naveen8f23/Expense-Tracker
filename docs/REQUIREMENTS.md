...
| MOB-5 | Ledger reaches the backend only over the owner's private Tailscale network (ADR-0002, ADR-0020) — the phone must have the Tailscale app installed, with **VPN On Demand set to "Always" for both Wi-Fi and Cellular (ADR-0025)**, so the tailnet tunnel is up automatically without a manual toggle before each use. This is a one-time device setting the owner performs themselves (same category as the Gmail OAuth consent click — not something the app can do on the owner's behalf). Per-app-only VPN (tunnel active only while Ledger runs) was investigated and found not possible on an unmanaged iPhone (ADR-0025). If the backend is unreachable, Ledger says so plainly and shows the last-known data, rather than hanging or pretending to be live. **Resolved 2026-07-20 (ADR-0028), closing the ADR-0026 gap:** the VM turned out to already have Tailscale installed, joined directly to the owner's own personal tailnet (`naveen8f23@gmail.com`) — not reachable through the relative's NAS/subnet-router route ADR-0026 had assumed at all. The owner's Mac and iPhone were switched from being guest devices on the relative's tailnet to being members of this same personal tailnet, making `turnny-vm` a direct peer of both. Ledger now points at the VM directly (`turnny-vm`, port 8000) via `ConnectionSettingsStore` — no code change was needed, since host/port were already a user-entered setting (I3). The relative's NAS/subnet-router path is no longer used for reaching the backend at all. |
