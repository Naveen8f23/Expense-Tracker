...
Syncs the current source tree to the VM, then runs pytest there against the VM's own venv.
Per docs/DECISIONS.md ADR-0017, the Ubuntu VM is the authoritative verification environment --
passing on macOS is necessary but not sufficient, since real divergence (ADR-0016, a Python
3.14/SQLAlchemy incompatibility that only exists on the VM's stock Python) has already been
found there once.
